...
const MesStocks = () => {
  ...
  const getStockStatus = (stock) => {
    if (stock.quantite_disponible === 0) {
      return {
        status: "rupture",
        color: "mes-stocks__status--rupture",
    ...
    if (stock.quantite_disponible <= stock.alerte_stock) {
      return {
        status: "alerte",
        color: "mes-stocks__status--alerte",
        text: "Stock faible",
      };
    }
    return {
      status: "normal",
      color: "mes-stocks__status--normal",
      text: "Stock normal",
    };
  };
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      setError(null);
      await Promise.all([fetchStocks(), fetchEntrepots()]);
    } catch (err) {
  ...
  const stocksFiltres = stocks.filter((stock) => {
    ...
    const matchSearch =
      stock.produit_detail?.nom_produit
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      stock.entrepot_detail?.nom_entrepot
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      stock.entrepot_detail?.ville
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
  ...
  const stats = {
    ...
    stock_normal: stocks.filter((s) => getStockStatus(s).status === "normal")
      .length,
    stock_alerte: stocks.filter((s) => getStockStatus(s).status === "alerte")
      .length,
    stock_rupture: stocks.filter((s) => getStockStatus(s).status === "rupture")
      .length,
  };
        ...
        <div className="mes-stocks__header-content">
          ...
          <div className="mes-stocks__header-actions">
            ...
            >
              <RefreshCw
                size={20}
                className={loading ? "mes-stocks__icon--spinning" : ""}
              />
              Actualiser
            </button>
            <button
      ...
      <div className="mes-stocks__stats">
        <div className="mes-stocks__stat-card">
          ...
          <div className="mes-stocks__stat-content">
            <h3 className="mes-stocks__stat-value">
              {formatNumber(stats.total_produits)}
            </h3>
        ...
        <div className="mes-stocks__stat-card">
          ...
          <div className="mes-stocks__stat-content">
            <h3 className="mes-stocks__stat-value">
              {formatNumber(stats.stock_normal)}
            </h3>
        ...
        <div className="mes-stocks__stat-card">
          ...
          <div className="mes-stocks__stat-content">
            <h3 className="mes-stocks__stat-value">
              {formatNumber(stats.stock_alerte)}
            </h3>
        ...
        <div className="mes-stocks__stat-card">
          ...
          <div className="mes-stocks__stat-content">
            <h3 className="mes-stocks__stat-value">
              {formatNumber(stats.stock_rupture)}
            </h3>
        ...
        {stocksFiltres.length === 0 ? (
          ...
          <div className="mes-stocks__list">
            {stocksFiltres.map((stock) => {
              ...
              return (
                <div
                  key={`${stock.produit}-${stock.entrepot}`}
                  className="mes-stocks__card"
                >
                  {/* Header de la carte */}
                  ...
                  <div className="mes-stocks__card-content">
                    <div className="mes-stocks__info-grid">
                      <div className="mes-stocks__info-item">
                        <span className="mes-stocks__info-label">
                          Quantité disponible
                        </span>
                        <span
                          className={`mes-stocks__info-value ${
                            stockStatus.status === "rupture"
                              ? "mes-stocks__info-value--error"
                              : ""
                          }`}
                        >
                          {formatNumber(stock.quantite_disponible)}{" "}
                          {stock.produit_detail?.unite}
                        </span>
                      </div>
                      ...
                      <div className="mes-stocks__info-item">
                        ...
                        <span className="mes-stocks__info-value">
                          {stock.entrepot_detail?.type_entrepot === "drivn_cook"
                            ? "DRIV'N COOK"
                            : "Fournisseur libre"}
                        </span>
                      </div>

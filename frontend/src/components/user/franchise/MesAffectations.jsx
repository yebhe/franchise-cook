...
const MesAffectations = () => {
  ...
  const getStatusColor = (statut) => {
    switch (statut) {
      case "programme":
        return "mes-affectations__badge--programme";
      case "en_cours":
        return "mes-affectations__badge--en_cours";
      case "termine":
        return "mes-affectations__badge--termine";
      case "annule":
        return "mes-affectations__badge--annule";
      default:
        return "mes-affectations__badge--default";
    }
  };
  ...
  const validateForm = () => {
    ...
    if (form.camion && form.date_debut) {
      const conflit = affectations.find((affectation) => {
        ...
        const memeDate =
          ...
          (form.date_fin &&
            form.date_debut <= affectation.date_debut &&
            form.date_fin >= affectation.date_debut);

        return (
          affectation.camion === parseInt(form.camion) &&
          memeDate &&
    ...
    if (form.emplacement && form.date_debut) {
      const emplacementOccupe = affectations.find((affectation) => {
        ...
        return (
          affectation.emplacement === parseInt(form.emplacement) &&
          memeDate &&
          affectation.statut !== "annule"
        );
      });
      ...
      if (emplacementOccupe) {
        errors.emplacement =
          "Cet emplacement est déjà occupé sur cette période";
      }
    }
  ...
  const handleDelete = async (affectationId) => {
    if (
      !window.confirm("Êtes-vous sûr de vouloir supprimer cette affectation ?")
    )
      return;
  ...
  const affectationsFiltrees = affectations.filter((affectation) => {
    ...
    const matchStatus =
      statusFilter === "tous" || affectation.statut === statusFilter;
    ...
    <div className="mes-affectations__container">
        ...
        <div className="mes-affectations__header-content">
          ...
          <div className="mes-affectations__header-actions">
            ...
            >
              <RefreshCw
                size={20}
                className={loading ? "mes-affectations__icon--spinning" : ""}
              />
              Actualiser
            </button>
            <button
                ...
                <div className="mes-affectations__card-header">
                  <div className="mes-affectations__card-info">
                    <div className="mes-affectations__card-title">
                      <Truck
                        size={18}
                        className="mes-affectations__card-icon"
                      />
                  ...
                  <span
                    className={`mes-affectations__badge ${getStatusColor(
                      affectation.statut
                    )}`}
                  >
                ...
                <div className="mes-affectations__card-content">
                  <div className="mes-affectations__info-grid">
                    <div className="mes-affectations__info-item">
                      <span className="mes-affectations__info-label">
                        Date de début
                      </span>
                    ...
                    {affectation.horaire_debut && (
                      <div className="mes-affectations__info-item">
                        ...
                        <span className="mes-affectations__info-value">
                          {formatTime(affectation.horaire_debut)} -{" "}
                          {formatTime(affectation.horaire_fin)}
                        </span>
                      </div>
                    )}
                  </div>
      ...
      {showForm && (
        <div className="mes-affectations__modal-overlay" onClick={resetForm}>
          <div
            className="mes-affectations__modal"
                ...
                <div className="mes-affectations__form-group">
                  ...
                  >
                    ...
                    {validationErrors.camion && (
                      <AlertTriangle
                        size={14}
                        className="mes-affectations__error-icon"
                      />
                    )}
                  </label>
                  <select
                    ...
                    required
                    className={`mes-affectations__select ${
                      validationErrors.camion
                        ? "mes-affectations__select--error"
                        : ""
                    }`}
                  >
                    ...
                    {camions
                      .filter(
                        (c) =>
                          c.statut === "disponible" || c.statut === "attribue"
                      )
                      .map((camion) => (
                        <option key={camion.id} value={camion.id}>
                          {camion.numero_camion} - {camion.marque}{" "}
                          {camion.modele}
                        </option>
                      ))}
                  </select>
                  {validationErrors.camion && (
                    <span className="mes-affectations__error-text">
                      {validationErrors.camion}
                    </span>
                  )}
                </div>
                ...
                >
                  {submitLoading ? (
                    <>
                      <Loader
                        size={16}
                        className="mes-affectations__icon--spinning"
                      />

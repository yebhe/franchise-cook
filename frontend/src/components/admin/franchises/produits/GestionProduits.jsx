...
export default function GestionProduits() {
  ...
  const getTotalStock = (produitId) => {
    ...
    return stocksProduit.reduce(
      (total, stock) => total + stock.quantite_disponible,
      0
    );
  };
  ...
  const filteredProduits = (produits || []).filter((produit) => {
    ...
    const matchesSearch = (produit.nom_produit || "")
      .toLowerCase()
      .includes(searchTerm.toLowerCase());

    const matchesCategorie =
      categorieFilter === "" || produit.categorie === parseInt(categorieFilter);


    const matchesEntrepot =
      entrepotFilter === "" ||
      stocks.some(
        (stock) =>
  ...
  return (
    <div className="gestion-produits__container">
        ...
        <div className="gestion-produits__header-content">
          <div className="gestion-produits__title-section">
            ...
            <p className="gestion-produits__subtitle">
              Gérer les ingrédients, plats préparés et boissons disponibles dans
              tous les entrepôts
            </p>
          </div>
          <button
      ...
      <div className="gestion-produits__info-banner">
        <Info size={20} />
        <div>
          <strong>Règle 80/20 :</strong> Les franchisés doivent commander 80%
      ...
      <div className="gestion-produits__content">
                  ...
                  <tbody>
                    {filteredProduits.length === 0 ? (
                      ...
                      filteredProduits.map((produit) => {
                        ...
                        const entrepotsDisponibles = getEntrepotsDisponibles(
                          produit.id
                        );
                        ...
                        return (
                          <tr
                            key={produit.id}
                            className="gestion-produits__table-row"
                          >
                            <td>
                            ...
                            <td>
                              <div className="gestion-produits__stock">
                                <span
                                  className={`gestion-produits__stock-badge ${
                                    totalStock > 0
                                      ? "gestion-produits__stock-badge--available"
                                      : "gestion-produits__stock-badge--empty"
                                  }`}
                                >
                                  {totalStock}
                                </span>
                            ...
                            <td>
                              <div className="gestion-produits__entrepots">
                                {entrepotsDisponibles.length > 0 ? (
                                  ...
                                  >
                                    {entrepotsDisponibles.length} entrepôt
                                    {entrepotsDisponibles.length > 1 ? "s" : ""}
                                  </button>
                                ) : (
                                  <span className="gestion-produits__no-stock">
                                    Aucun stock
                                  </span>
                                )}
                              </div>
        ...
        {showForm && (
          <div className="gestion-produits__modal-overlay" onClick={resetForm}>
            <div
              className="gestion-produits__modal"
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="gestion-produits__form">
                ...
                <div className="gestion-produits__form-info">
                  <Info size={16} />
                  <small>
                    Ce produit sera disponible dans les entrepôts via la gestion
                    des stocks. La règle 80/20 s'applique selon le type
                    d'entrepôt où il est stocké.
                  </small>
                </div>
        ...
        {showStockModal && selectedProduitId && (
          <div
            className="gestion-produits__modal-overlay"
              ...
              <div className="gestion-produits__stock-details">
                {getStocksProduit(selectedProduitId).map((stock) => {
                  const entrepot = entrepots.find(
                    (e) => e.id === stock.entrepot
                  );
                  return (
                    <div
                      key={stock.id}
                      className="gestion-produits__stock-item"
                    >
                      <div className="gestion-produits__stock-item-header">
                        <strong>{entrepot?.nom_entrepot}</strong>
                        <span
                          className={`gestion-produits__entrepot-type ${
                            entrepot?.type_entrepot === "drivn_cook"
                              ? "gestion-produits__entrepot-type--drivn"
                              : "gestion-produits__entrepot-type--libre"
                          }`}
                        >
                          {entrepot?.type_entrepot === "drivn_cook"
                            ? "Driv'n Cook (80%)"
                            : "Fournisseur libre (20%)"}
                        </span>
                      </div>
                      <div className="gestion-produits__stock-quantities">
                        ...
                        <span>
                          Total:{" "}
                          {stock.quantite_disponible + stock.quantite_reservee}
                        </span>
                      </div>
                    </div>
                  );

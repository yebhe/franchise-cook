...
export default function GestionEmplacements() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setEmplacements(
        Array.isArray(emplacementsRes.data) ? emplacementsRes.data : []
      );
      setFranchises(
        Array.isArray(franchisesRes.data) ? franchisesRes.data : []
      );
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formData = {
        ...form,
        tarif_journalier: form.tarif_journalier
          ? parseFloat(form.tarif_journalier)
          : null,
  ...
  const handleEdit = (emplacement) => {
    setForm({
      ...
      franchises_autorisees:
        emplacement.franchises_autorisees ||
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (
        ...
        err.response?.data?.affectations_actives
      ) {
        const affectations = err.response.data.affectations_actives;
        const message =
          `Impossible de supprimer : ${affectations.length} affectation(s) en cours:\n\n` +
          affectations
            .map(
              (aff) =>
        ...
        if (window.confirm(message)) {
          try {
            ...
            setMessage(
              "Emplacement supprimé avec succès (affectations annulées)"
            );
            fetchData();
          } catch (forceErr) {
  ...
  const filteredEmplacements = emplacements.filter((emplacement) => {
    ...
    const matchesType =
      typeFilter === "" || emplacement.type_zone === typeFilter;
  ...
  return (
    <div className="gestion-emplacements__container">
      ...
      <div className="gestion-emplacements__content">
                  ...
                  <tbody>
                    {filteredEmplacements.length === 0 ? (
                      ...
                      filteredEmplacements.map((emplacement) => (
                        <tr
                          key={emplacement.id}
                          className="gestion-emplacements__table-row"
                        >
                          <td>
                              ...
                              <div className="gestion-emplacements__emplacement-meta">
                                {emplacement.tarif_journalier ? (
                                  <span className="gestion-emplacements__tarif">
                                    <Euro size={14} />{" "}
                                    {emplacement.tarif_journalier}€/jour
                                  </span>
                                ) : (
                                  <span className="gestion-emplacements__tarif-free">
                                    Gratuit
                                  </span>
                                )}
                          ...
                          <td>
                            <span
                              className={`gestion-emplacements__badge gestion-emplacements__badge--${emplacement.type_zone}`}
                            >
        ...
        {showForm && (
          <div
            className="gestion-emplacements__modal-overlay"
            onClick={resetForm}
          >
            <div
              className="gestion-emplacements__modal"
            ...
            >
              ...
              <form
                ...
                className="gestion-emplacements__form"
              >
                <div className="gestion-emplacements__form-group">
                  <label htmlFor="nom_emplacement">
                ...
                <div className="gestion-emplacements__form-group">
                  <label htmlFor="adresse">
                    Adresse complète *
                    {isGoogleMapsLoaded && (
                      <span className="gestion-emplacements__maps-badge">
                        Google Maps
                      </span>
                    )}
                  </label>
                  <div className="gestion-emplacements__address-group">
                    <div className="gestion-emplacements__address-wrapper">
                      <MapPin
                        className="gestion-emplacements__address-icon"
                        size={16}
                      />
                      <input
                        ref={addressInputRef}
                        ...
                        className={`gestion-emplacements__input gestion-emplacements__input--with-icon ${
                          addressValidated
                            ? "gestion-emplacements__input--validated"
                            : ""
                        }`}
                      ...
                      {addressValidated && (
                        <CheckCircle
                          className="gestion-emplacements__address-check"
                          size={16}
                        />
                      )}
                    </div>
                ...
                <div className="gestion-emplacements__form-group">
                  ...
                  <div className="gestion-emplacements__input-wrapper">
                    <Building2
                      className="gestion-emplacements__input-icon"
                      size={16}
                    />
                    <select
                      id="type_zone"
                ...
                <div className="gestion-emplacements__form-row">
                  <div className="gestion-emplacements__form-group">
                    <label htmlFor="tarif_journalier">
                      Tarif journalier (€)
                    </label>
                    <div className="gestion-emplacements__input-wrapper">
                      <Euro
                  ...
                  <div className="gestion-emplacements__form-group">
                    <label htmlFor="horaires_autorises">
                      Horaires autorisés
                    </label>
                    <div className="gestion-emplacements__input-wrapper">
                      <Clock
                ...
                <div className="gestion-emplacements__franchises-section">
                  ...
                  >
                    <Users size={18} />
                    <span>
                      Franchises autorisées ({form.franchises_autorisees.length}
                      )
                    </span>
                    {showFranchisesPanel ? (
                      <ChevronUp size={18} />
                      ...
                      {franchises.length === 0 ? (
                        ...
                        <div className="gestion-emplacements__franchises-grid">
                          {franchises.map((franchise) => (
                            <div
                              key={franchise.id}
                              className="gestion-emplacements__franchise-item"
                            >
                              <input
                                type="checkbox"
                                id={`franchise-${franchise.id}`}
                                checked={form.franchises_autorisees.includes(
                                  franchise.id
                                )}
                                onChange={() =>

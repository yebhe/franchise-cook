...
export default function GestionFranchises() {
  ...
  const [validationAddressLoading, setValidationAddressLoading] =
    useState(false);
  ...
  const confirmerValidation = async () => {
    ...
    setValidationLoading((prev) => ({
      ...prev,
      [franchiseToValidate.id]: true,
    }));

    try {
      const response = await apiClient.post(
        `api/franchises/${franchiseToValidate.id}/valider/`,
        {
  ...
  const regenererLienPaiement = async (franchiseId) => {
    ...
    try {
      const response = await apiClient.post(
        `api/franchises/${franchiseId}/regenerer-lien/`
      );
      setMessage("Nouveau lien de paiement généré et envoyé !");
      fetchFranchises();
    } catch (err) {
      setError(
        err.response?.data?.error || "Erreur lors de la génération du lien"
      );
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Êtes-vous sûr de vouloir supprimer cette franchise ?"))
      return;
  ...
  const filteredFranchises = franchises.filter((franchise) => {
    ...
    const matchesStatus =
      statusFilter === "" || franchise.statut === statusFilter;
  ...
  return (
    <div className="gestion-franchises__container">
                  ...
                  <tbody>
                    {filteredFranchises.length === 0 ? (
                      ...
                      filteredFranchises.map((franchise, i) => (
                        <tr
                          key={franchise.id}
                          className="gestion-franchises__table-row"
                        >
                          <td>
                          ...
                          <td>
                            <div className="gestion-franchises__user-info">
                              <span>
                                {franchise.user_first_name || "N/A"}{" "}
                                {franchise.user_last_name || "N/A"}
                              </span>
                          ...
                          <td>{getStatusBadge(franchise)}</td>
                          <td>
      ...
      {showValidationModal && franchiseToValidate && (
        <div
          className="gestion-franchises__modal-overlay"
          onClick={() => setShowValidationModal(false)}
        >
          <div
            className="gestion-franchises__modal gestion-franchises__modal--validation"
            ...
            <div className="gestion-franchises__validation-content">
              <div className="gestion-franchises__franchise-summary">
                ...
                <p>
                  ...
                  {franchiseToValidate.user_first_name}{" "}
                  {franchiseToValidate.user_last_name}
                </p>
                <p>
                  <strong>Email :</strong> {franchiseToValidate.user_email}
                </p>
                <p>
                  <strong>Localisation :</strong> {franchiseToValidate.ville},{" "}
                  {franchiseToValidate.code_postal}
                </p>
                <p>
                  <strong>Droit d'entrée :</strong>{" "}
                  {franchiseToValidate.droit_entree}€
                </p>
              </div>
              ...
              <div className="gestion-franchises__validation-warning">
                ...
                <div>
                  ...
                  <p>
                    En validant cette franchise, un email sera automatiquement
                    envoyé au franchisé avec un lien de paiement Stripe pour
                    régler le droit d'entrée de{" "}
                    {franchiseToValidate.droit_entree}€.
                  </p>
                </div>
              </div>
      ...
      {showForm && (
        <div className="gestion-franchises__modal-overlay" onClick={resetForm}>
          <div
            className="gestion-franchises__modal"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="gestion-franchises__modal-header">
            ...
            <form onSubmit={handleSubmit} className="gestion-franchises__form">
              ...
              {!editingId && (
                <div className="gestion-franchises__engagement-notice">
                  ...
                  <div className="gestion-franchises__engagement-text">
                    ...
                    <p>
                      L'utilisateur s'engage à payer un droit d'entrée de{" "}
                      <strong>50 000€</strong> et une redevance de{" "}
                      <strong>4%</strong> du chiffre d'affaires.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="gestion-franchises__form-group">
                <label htmlFor="adresse">
                  Adresse complète
                  {isGoogleMapsLoaded && (
                    <span className="gestion-franchises__maps-badge">
                      Google Maps
                    </span>
                  )}
                </label>
                <div className="gestion-franchises__address-input-group">
                  <div className="gestion-franchises__address-input-wrapper">
                    <MapPin
                      className="gestion-franchises__address-icon"
                      size={16}
                    />
                    <input
                      ref={addressInputRef}
                      ...
                      value={form.adresse}
                      onChange={handleChange}
                      className={`gestion-franchises__input gestion-franchises__input--with-icon ${
                        addressValidated
                          ? "gestion-franchises__input--validated"
                          : ""
                      }`}
                    ...
                    {addressValidated && (
                      <CheckCircle
                        className="gestion-franchises__address-check"
                        size={16}
                      />
                    )}
                  </div>
